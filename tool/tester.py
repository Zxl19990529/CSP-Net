...
from basic_function import get_coco2014_colors
...
class Tester(object):
    ...
    def test(self, data_loader=None):
        ...
        if self.configer.get('dataset') == 'ScribbleCOCO2014':
            colors = get_coco2014_colors()
        else:
        ...
        for j, data_dict in enumerate(self.test_loader):
            ...
            with torch.no_grad():
                ...
                # if self.configer.get('test', 'mode') == 'ms_test':
                outputs = self.ms_test(inputs)

                # outputs = self.dense_crf_process(inputs, outputs)

                # print("val mode:", self.configer.get('test', 'mode'))
                # print("val mode:", self.configer.get('test', 'mode'))
                # print("val mode:", self.configer.get('test', 'mode'))
                # print("val mode:", self.configer.get('test', 'mode'))
                # print("val mode:", self.configer.get('test', 'mode'))
    ...
    def ss_test(self, inputs, scale=1):
        if isinstance(inputs, torch.Tensor):
            ...
            # replicas = nn.parallel.replicate(self.seg_net.module, [0])
    ...
    def ms_test(self, inputs):
        if isinstance(inputs, torch.Tensor):  
            ...
            print("inputs[0] sz:", inputs[0].size())
